// Country dial codes for the signup mobile-number selector. `iso` is the short
// badge shown on the left (US/CA is a combined NANP entry), `name` the full
// country name, `dial` the calling code. POPULAR_ISOS are pinned to the top of
// the list; the rest follow alphabetically by name.
export type Country = { iso: string; name: string; dial: string };

export const POPULAR_ISOS = ["US/CA", "IN", "GB", "AU", "AE"];

export const COUNTRIES: Country[] = [
  { iso: "US/CA", name: "United States / Canada", dial: "+1" },
...
/** Countries ordered with the popular ones pinned first, then alphabetical. */
export const ORDERED_COUNTRIES: Country[] = (() => {
  const byIso = new Map(COUNTRIES.map((c) => [c.iso, c]));
  const popular = POPULAR_ISOS.map((iso) => byIso.get(iso)).filter(
    (c): c is Country => Boolean(c)
  );
  const popularSet = new Set(POPULAR_ISOS);
